...
class TestController extends ControllerBase
{
	...
	public function postQuestions()
	{
		...
		$Sus=\Auth::user()->userIsSuspended;
		if($Sus == 'Y')
		{
			$data=array('msg'=>"<div class='alert alert-danger'>Your disqualified for further round</div>");
		...
		if($todayFrom <= $todayNow && $todayTo >= $todayNow)
		{
			$data=array('msg'=>"<div class='alert alert-danger'>Today Session completed. Next round starts at 10p.m</div>");
			echo json_encode($data);
			return;
		}
    	/*---------------End check current date between 8pm to 10pm--------------------------------*/
		$userId=\Auth::user()->id;
		$Qids = \DB::table('today_question')->select('question_id')->get();
			if($Qids)
			{
				foreach($Qids as $quId)
				{
					$QId[]=$quId->question_id;
				}
				$strQId = implode(',',$QId);
				//sql for registered question which is unanswered
				$nos	="select * from participant_stat where question_id in ($strQId) AND participant_id = $userId";
				$nos	= \DB::select($nos);
				$nos	= count($nos);
				$nos	= ($nos) ? $nos : 1;
				$sql 	= "select * from participant_stat where question_id in ($strQId) AND participant_id = $userId and answered = 0 ";
				$exe 	=  \DB::select($sql);
						if($exe)
						{
							$questionId=$exe[0]->question_id;
							$r_sql=" select * from game_question where id = $questionId";
						}
						else
						{
							// sql for question which is answered
							$answeredQuestion =  \DB::table('participant_stat')->whereIn('question_id',$QId)->where('participant_id','=',$userId)->where('answered','=',1)->get();
							$count=count($answeredQuestion);
								if($count == 2)
								{
									$msg="<div class='alert alert-success'>Congratulations you have completed all the questions for the day. Login  after 10 PM to see the next set of questions</div>";

								}
								elseif($count == 1)
								{
									$questionId=$answeredQuestion[0]->question_id;
									$filteredIds=array_diff($QId,array($questionId));
										if($filteredIds)
										{
											$randId=$filteredIds[array_rand($filteredIds,1)];
											$r_sql=" select * from game_question where id = $randId";
											$nos=2;
										}
										else
										{
											$msg="<div class='alert alert-warning'>Today Questions are not released</div>";
										}
									
								}
								else
								{
									$randId=$QId[array_rand($QId,1)];
									$r_sql="select * from game_question where id = $randId ";
								}
						}
			}
			...
			{
				$msg="<div class='alert alert-danger'>Today Questions are not released</div>";
			}
			 if(isset($r_sql))
			 {
			 	...
			 	$res=\DB::select($r_sql);
			 	$questionId=$res[0]->id;
			 	$pStat=\DB::table('participant_stat')->where('question_id','=',$questionId)->where('participant_id','=',$userId)->get();
				 	if($pStat)
				 	{
				 		$participant_id=$pStat[0]->id;
				 		$sTime=$pStat[0]->start_time;
				 	}
				 	else
				 	{	
				 		$sTime=Date('Y-m-d H:i:s');
				 		$participant_id=\DB::table('participant_stat')
	 										->insertGetId(array(
				...
				$data=array('test'=>$res[0],'participant_id'=>$participant_id,'sTime'=>$sTime,'nTime'=>Date('Y-m-d H:i:s'),'nos'=>$nos);
				return json_encode($data);
			 }
			 elseif(isset($msg))
			 {
